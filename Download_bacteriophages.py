...
def bachplip_metadata():
    ...
    df.to_csv(input("Write *.csv filename with metadata: "))
...
def remove_to_csv():
    ...
    metadata_df = pd.read_csv('seq.csv')
...
def new_phanotate():
    ...
    SeqIO.write(short_sequences, input('Write new *.fasta filename: '), "fasta")
        ...
        for record in GenBank.parse(handle):
            if 'full length' in record.comment:
                ...
                for col in df:
                    ...
                    df.to_csv(input('Write filename with new data: '))
...
def ictv_to_df():
    ...
    new_df.to_csv(input('Write *.csv ICTV filename: '))
...
def ictv_to_csv():
    ...
    df2.to_csv(input('Write filename with new data: '))


def count_n():

    metadata_df = pd.read_csv(input("Write *.csv filename with metadata: "))
    ncount_list, accession_list = countN_meta()
    df = pd.DataFrame(np.column_stack([accession_list, ncount_list]), columns=['Accession', 'Ncount'])
    df1 = (df.merge(metadata_df, on='Accession', how='left'))
    df1.to_csv(input('Write filename with new data: '))